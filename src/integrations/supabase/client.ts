...
export const refreshToken = async (): Promise<boolean> => {
  try {
    sessionLogger.logEvent('refresh_token_started');
    const { data, error } = await supabase.auth.refreshSession();
    if (error) {
      console.error('Error refreshing token:', error);
      sessionLogger.logEvent('refresh_token_failed', error);
      return false;
    }
    sessionLogger.logEvent('refresh_token_succeeded');
    return !!data.session;
  } catch (err) {
    console.error('Exception during token refresh:', err);
    sessionLogger.logEvent('refresh_token_exception', err);
    return false;
  }
};
...
export const isSessionValid = (session: any): boolean => {
  ...
  try {
    ...
    if (session.expires_at) {
      ...
      sessionLogger.logEvent('session_validity_check', { 
        isValid, 
        expiresAt, 
        now: new Date()
      });
    ...
    return hasToken; // Fallback: if there's an access token, consider it valid
  } catch (err) {
    console.error('Error checking session validity:', err);
    sessionLogger.logEvent('session_validity_check_error', err);
    return false;
  }
};

// Remove the duplicate sessionLogger definition - we're importing it from utils now
