...
-- Enable RLS on teachers table (fixing "Policy Exists RLS Disabled" error)
...
-- Drop existing security definer views and replace with proper functions
DROP VIEW IF EXISTS public.school_analytics_summary;
DROP VIEW IF EXISTS public.most_studied_topics;
DROP VIEW IF EXISTS public.student_weekly_study_time;
DROP VIEW IF EXISTS public.session_query_counts;
DROP VIEW IF EXISTS public.school_performance_metrics;
DROP VIEW IF EXISTS public.teacher_performance_metrics;
